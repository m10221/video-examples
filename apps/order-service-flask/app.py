...
# Get environment name from environment variables (generic default)
environment_name = os.environ.get("ENVIRONMENT", "moss-demo-environment")
...
# Configure exporter (default to host collector when running standalone in Docker)
otlp_endpoint = os.environ.get("OTEL_EXPORTER_OTLP_ENDPOINT", "http://host.docker.internal:4317")
...
@app.route('/')
def index():
    return "Shared Order Service Demo. Use /checkout to simulate orders."
